...
# generated by the cryptogen tool and output a docker-compose-e2e-template.yaml specific to this
# configuration
# THIS WAS UPDATED TO BE PART OF THE PROJECT
...
function generateChannelArtifacts() {
  ...
  echo "##########################################################"
  echo $PWD 
  cd configuration #for unkown reason this has to be add
  ...
  if [ "$CONSENSUS_TYPE" == "solo" ]; then
    configtxgen -profile ThreeOrgOrdererGenesis -channelID byfn-sys-channel -outputBlock ./channel-artifacts/genesis.block
    echo 'solo done'
  ...
  set -x
  echo $PWD 
  configtxgen -profile ThreeOrgChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ThreeOrgChannel -outputAnchorPeersUpdate ./channel-artifacts/BuyerMSPanchors.tx -channelID $CHANNEL_NAME -asOrg BuyerMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ThreeOrgChannel -outputAnchorPeersUpdate ./channel-artifacts/SellerMSPanchors.tx -channelID $CHANNEL_NAME -asOrg SellerMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ThreeOrgChannel -outputAnchorPeersUpdate ./channel-artifacts/ObserverMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ObserverMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
